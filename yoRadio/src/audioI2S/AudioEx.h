...
class Audio {
  ...
  public:
    ...
    void             setBalance(int8_t balance); // balance -16...16
  ...
  private:
    // ------- PRIVATE MEMBERS ----------------------------------------
    uint32_t                 lastBitrate = 0;         // last known bitrate
    uint32_t                 lastBitrateSendTime = 0; // last time bitrate sent to Yoradio
  ...
  private:
    ...
    enum : int { YORADIO_SYNC_FAST = 1000, YORADIO_SYNC_MEDIUM = 3000, YORADIO_SYNC_SLOW = 12000}; // detemrines how often send data to yoradio UI (in ms)
